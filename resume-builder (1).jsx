...
function RS({ title, children, accent = "#1a1a1a" }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ fontSize: "10pt", fontWeight: 700, letterSpacing: 0.5, borderBottom: `1.5px solid ${accent}`, paddingBottom: 2, marginBottom: 5 }}>{title}</div>
...
function SS({ title, children }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: "8pt", letterSpacing: 2, color: "#64748b", marginBottom: 6, fontWeight: 700 }}>{title}</div>
      {children}
    </div>
  );
}
...
// ── PDF Export ────────────────────────────────────────────────────────────────
function exportPDF(ref, name) {
  const node = ref.current;
  if (!node) return;
  const win = window.open("", "_blank");
  if (!win) { alert("Please allow popups to download PDF."); return; }
  win.document.open();
  win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${name || "Resume"}</title>
<style>
  @page { margin: 0; size: A4 portrait; }
  html, body { margin: 0; padding: 0; background: #fff; }
  * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  a { color: inherit; }
  .page-block { width: 210mm; min-height: 297mm; overflow: hidden; page-break-after: always; }
</style>
...
function PaginatedResume({ data, template }) {
  // We render the full content in a hidden off-screen div, measure it,
  // then display it sliced into fixed-height page windows.
  const measureRef = useRef(null);
  const [totalH, setTotalH] = useState(PAGE_H);

  // Remeasure whenever data or template changes
  useEffect(() => {
    ...
    // Also watch for async layout changes (images, fonts)
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      {/* Hidden measurement layer — full content rendered at true A4 width */}
      ...
      {Array.from({ length: numPages }).map((_, pi) => (
        <div key={pi} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <div style={{
            ...
            {/* Content shifted up by page offset */}
            <div style={{ position: "absolute", top: -(pi * PAGE_H), left: 0, width: PAGE_W }}>
              <TemplateComp data={data} />
            </div>
          ...
          {pi < numPages - 1 && (
            <div style={{
              ...
              {/* Left rule */}
              <div style={{ position: "absolute", left: 0, right: 0, height: 1, background: "#c4cdd8" }} />
              {/* Label chip */}
              <div style={{
                position: "relative",
                background: "#dce3eb",
            ...
            <div key={i} style={{ marginBottom: 8 }}>
              ...
              {e.bullets?.filter(b => b?.trim()).length > 0 && (
                <ul style={{ margin: "4px 0 0 18px", padding: 0 }}>
                  {e.bullets.filter(b => b?.trim()).map((b, j) => <li key={j} style={{ marginBottom: 2 }}><strong>•</strong> {b}</li>)}
                </ul>
              )}
            </div>
          ))}
          ...
          {projects.filter(p => p.name).map((proj, i) => (
            <div key={i} style={{ marginBottom: 7 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                {proj.link && <RLink href={proj.link} label={proj.link} />}
              </div>
              {proj.tech && <div style={{ fontSize: "9pt", color: "#555" }}>Tech: {proj.tech}</div>}
              {proj.bullets?.filter(b => b?.trim()).length > 0 && (
                <ul style={{ margin: "4px 0 0 18px", padding: 0 }}>
                  {proj.bullets.filter(b => b?.trim()).map((b, j) => <li key={j} style={{ marginBottom: 2 }}><strong>•</strong> {b}</li>)}
                </ul>
              )}
            </div>
          ))}
      ...
      {certifications.some(c => c.name) && (
        <RS title="CERTIFICATIONS">
          <ul style={{ margin: 0, paddingLeft: 16 }}>
            {certifications.filter(c => c.name).map((c, i) => <li key={i} style={{ marginBottom: 2 }}>• {c.name}{c.issuer ? ` - ${c.issuer}` : ""}{c.year ? ` (${c.year})` : ""}</li>)}
          </ul>
        </RS>
      )}
          ...
          <SS title="CERTIFICATIONS">
            {certifications.filter(c => c.name).map((c, i) => (
              <div key={i} style={{ marginBottom: 5, fontSize: "9pt" }}>
                <div style={{ color: "#ecf0f1" }}>{c.name}</div>
                <div style={{ color: "#bdc3c7", fontSize: "8pt" }}>{c.issuer}{c.year ? ` · ${c.year}` : ""}</div>
              </div>
            ))}
          </SS>
        )}
      ...
      <div style={{ flex: 1, padding: "28px 24px" }}>
        ...
        {projects.some(p => p.name) && (
          <RS title="PROJECTS" accent="#2c3e50">
            {projects.filter(p => p.name).map((proj, i) => (
              <div key={i} style={{ marginBottom: 7 }}>
                <strong>{proj.name}</strong>{proj.tech && <span style={{ fontSize: "9pt", color: "#555" }}> · {proj.tech}</span>}
                {proj.bullets?.filter(b => b?.trim()).length > 0 && (
                  <ul style={{ margin: "4px 0 0 18px", padding: 0 }}>
                    {proj.bullets.filter(b => b?.trim()).map((b, j) => <li key={j} style={{ marginBottom: 2 }}><strong>•</strong> {b}</li>)}
                  </ul>
    ...
    <div style={{ fontFamily: "'Arial Narrow', Arial, sans-serif", fontSize: "9.5pt", color: "#111", padding: "24px 36px", background: "#fff" }}>
      ...
      {experience.some(e => e.role) && (<><MH title="EXPERIENCE" />{experience.filter(e => e.role).map((e, i) => (
        <div key={i} style={{ marginBottom: 8 }}>
          ...
          {e.bullets?.filter(b => b?.trim()).map((b, j) => <div key={j} style={{ paddingLeft: 12 }}><strong>•</strong> {b}</div>)}
        </div>
      ))}</>)}
      ...
      {projects.some(p => p.name) && (<><MH title="PROJECTS" />{projects.filter(p => p.name).map((proj, i) => (
        <div key={i} style={{ marginBottom: 6 }}>
          ...
          {proj.bullets?.filter(b => b?.trim()).map((b, j) => <div key={j} style={{ paddingLeft: 12 }}><strong>•</strong> {b}</div>)}
        </div>
      ))}</>)}
      {certifications.some(c => c.name) && (<><MH title="CERTIFICATIONS" />{certifications.filter(c => c.name).map((c, i) => <div key={i} style={{ marginBottom: 3 }}>• {c.name}{c.issuer ? ` — ${c.issuer}` : ""}</div>)}</>)}
...
// ── UPLOAD MODAL ──────────────────────────────────────────────────────────────
// API key handled server-side via /api/groq proxy
  ...
  const parseResume = async () => {
    ...
    try {
      ...
      if (!resumeText || resumeText.length < 30)
        throw new Error("Could not extract readable text. Try a text-based PDF or TXT file.");
...
export default function ResumeBuilder() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: C.appBg, fontFamily: "'Segoe UI', system-ui, sans-serif", overflow: "hidden" }}>
      ...
      {/* ── Icon Sidebar ── */}
      <div style={{ width: 58, background: C.sidebarBg, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 12, gap: 2, boxShadow: "1px 0 4px rgba(0,0,0,0.05)" }}>
        ...
        {sideNav.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} title={s.label}
            style={{ width: 42, height: 42, border: activeSection === s.id ? `1.5px solid ${C.accentBorder}` : "1.5px solid transparent", cursor: "pointer", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", background: activeSection === s.id ? C.accentLight : "transparent", color: activeSection === s.id ? C.accent : C.textLight, transition: "all 0.15s" }}>
            <svg width={17} height={17} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={s.icon} /></svg>
          </button>
        ))}
      </div>

      {/* ── Form Panel ── */}
      <div style={{ width: 330, background: C.panelBg, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", background: C.previewBg, padding: "28px 0 60px", display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <div ref={previewRef} style={{ position: "fixed", left: -9999, top: 0, width: PAGE_W, pointerEvents: "none", zIndex: -1 }}>
            {activeTemplate === "A" && <TemplateA data={data} />}
            {activeTemplate === "B" && <TemplateB data={data} />}
            {activeTemplate === "C" && <TemplateC data={data} />}
          </div>

          {/* Visible paginated preview — scaled to fit panel */}
          <div style={{ transform: "scale(0.80)", transformOrigin: "top center", width: PAGE_W }}>
            <PaginatedResume data={data} template={activeTemplate} />
          </div>
        </div>
      </div>
    </div>
...
function CertsForm({ certifications, setData, IS, CS }) {
  ...
  const add = () => setData(p => ({ ...p, certifications: [...p.certifications, { name:"",issuer:"",year:"" }] }));
    ...
    <div>
      ...
      {certifications.map((c, i) => (
        <div key={i} style={CS}>
          ...
          {[["Certificate Name","name"],["Issuer / Organization","issuer"],["Year","year"]].map(([lbl,k]) => (
            <div key={k} style={{ marginBottom: 7 }}><FL>{lbl}</FL><input value={c[k]} onChange={e=>upd(i,k,e.target.value)} style={IS} /></div>
          ))}
        </div>
      ))}
